...
token_type PrimaryCommands::SendGetToken(stream_id stream_id) {
  ...
  carla::Buffer buf(reinterpret_cast<carla::Buffer::value_type *>(&stream_id),
...
void PrimaryCommands::SendEnableForROS(carla::streaming::detail::stream_actor_id_type stream_actor_id) {
  ...
  if (it != _servers.end()) {
    carla::Buffer buf(reinterpret_cast<carla::Buffer::value_type *>(&stream_actor_id),
                      static_cast<size_t>(sizeof(stream_actor_id)));
